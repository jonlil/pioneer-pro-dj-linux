pub mod Message {
    ...
    pub type TermDJMessageType = Vec<u8>;
    ...
    impl <'a>IntoTermDJMessage for ApplicationBroadcast<'a> {
        fn compose(&self) -> Vec<TermDJMessageType> {
            vec![
    ...
    impl <'a>Into<TermDJMessageType> for ApplicationBroadcast<'a> {
        fn into(self) -> TermDJMessageType {
    ...
    impl <'a>IntoTermDJMessage for DiscoveryInitial<'a> {
        fn compose(&self) -> Vec<TermDJMessageType> {
            vec![
                SOFTWARE_IDENTIFICATION.to_vec(),
                vec![0x00,0x00],
    ...
    impl <'a>Into<TermDJMessageType> for DiscoveryInitial<'a> {
        fn into(self) -> TermDJMessageType {
            self.compose().into_iter().flatten().collect()
        }
    }
    ...
    impl <'a>IntoTermDJMessage for DiscoverySequence<'a> {
        fn compose(&self) -> Vec<TermDJMessageType> {
            vec![
                SOFTWARE_IDENTIFICATION.to_vec(),
                vec![0x02, 0x00],
    ...
    impl <'a>Into<TermDJMessageType> for DiscoverySequence<'a> {
        fn into(self) -> TermDJMessageType {
            self.compose().into_iter().flatten().collect()
        }
    }

    trait IntoTermDJMessage {
        fn compose(&self) -> Vec<TermDJMessageType>;
    }
}
